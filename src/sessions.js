...
class Sessions extends Component {
    ...
    render() {
        ...
        const session = this.props.sessions.map(session => {
            ...
            const time = session.time.replace(/-/g, "/").slice(11, 16)
            const date = session.date.replace(/-/g, "/").split("T")[0]
            const timeInt = parseInt(time)
            return (<><div key={session.id} onClick={() => this.SessionClicked(session.id)}><strong>{session.student.name}:</strong> </div><p> {session.subject} with {session.instructor.name} on {date} at {timeInt >= 12 ? `${time % 12}pm` : `${time % 12}am`}</p></>)
        })
